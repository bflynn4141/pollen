...
4. Use the verified Worker origin and synchronize the Worker and GitHub admin secrets.
...
| 5 | Establish a verified production origin | Complete | `pollen-api.bflynn4141.workers.dev` is controlled and responds correctly. `pollen.id` is an unrelated product and is excluded. |
...
- [x] `pollen-api.bflynn4141.workers.dev` returns valid TLS and Worker responses without Cloudflare origin errors.
