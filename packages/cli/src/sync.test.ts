...
describe('syncToNeon batching', () => {
  ...
  it('re-syncs sessions within the 7-day overlap window behind the watermark', async () => {
    ...
    // one tuple = SESSIONS_COLUMNS.length params; second row's session_id
    // sits at index [columns] (39 after the start_source column landed)
    const sessionIds = [inserts[0].params[0], inserts[0].params[39]]
